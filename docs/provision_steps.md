...
# Steps specific to worker

To start and stop delayed job from monit we use capistrano tasks (task
triggered on server and executed on server) so we need to be able to ssh locally
on the server, so copy pem file and check ssh
```
# on local machine
cp ~/.ec2/singapore-movebase-test-01-keypair.pem .
bin/upload.sh staging-worker singapore-movebase-test-01-keypair.pem -ssh
rm singapore-movebase-test-01-keypair.pem

# on remote server
mkdir .ec2
mv singapore-movebase-test-01-keypair.pem ~/movebase
ln -s /home/ubuntu/movebase/singapore-movebase-test-01-keypair.pem ~/.ec2

# generate keys for local ssh
ssh-keygen

# map public ip or dns name to localhost in ~/.ssh/config
Host 18.232.175.34
  Hostname localhost

Host staging-worker.movebase.com
  Hostname localhost

# on remote server try to ssh locally
ssh -i .ec2/singapore-movebase-test-01-keypair.pem 18.232.175.34
```

Test that jobs are started when you reboot (they will be started by monit)

There are perl scripts wich uses other modules so we need to install them
```
cpan
...
Before testing you should migrate database
```
cd movebase/current
bundle exec rails db:migrate
```
if you use small instance 1GB RAM than you might decrease number of workers in
`config/deploy.rb`

and commit you changes in some branch (since we have updated staging ip address)
```
git checkout -b test_new_staging
git add .
git commit -am'Use IP of new staging'
git push origin test_new_staging -f --set-upstream

cap staging deploy ROLES=worker BRANCH=test_new_staging
```
...
sudo monit summary
tail movebase/current/log/monit.log
tail movebase/current/log/delayed_job.log
```
Check logs
```
sudo reboot
tail movebase/current/log/cron/*
```
...
```
You should see nginx page on http://movebase-test-01.com/
...
```
# sudo ln -s /home/ubuntu/movebase/current/config/etc/systemd/system/puma.service /etc/systemd/system
sudo cp /home/ubuntu/movebase/current/config/etc/systemd/system/puma.service /etc/systemd/system
...
sudo ln -s /home/ubuntu/movebase/current/config/etc/nginx/sites-enabled/nginx_puma /etc/nginx/sites-enabled
```
